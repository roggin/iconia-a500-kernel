...
/* reserve this, the ConfigChecksum should be updated */
/* #define ConfigChecksum           6160596    T10A_0120.cfg */
/* #define ConfigChecksum           613770     T10A_0306.cfg */
		...
		if (buffer[0] == 0x01) {
			if (buffer[1] != 0x00) {
				if ((buffer[1] & 0x08)==0x08) {
					ConfigError = 1;
					mxt_debug(DEBUG_DETAIL, "release\n");
					up(&mxt->sema);
...
{
	debug = myatoi(buf);
	mxt_debug(DEBUG_ERROR, "maXTouch1386: debug is %d\n", debug);
	return n;
}
...
static ssize_t FirmwareVersion_store(struct kobject *kobj, struct kobj_attribute *attr, const char * buf, size_t n)
{
	...
	for(i = 0; i < MaxTableSize; i++) {
		if (OBJTable[i][0] == Type) {
			if (Type == TOUCH_MULTITOUCHSCREEN_T9) {
				Table[1] = OBJTable[i][1];         /*  table is changed */
...
/* boot argument is for determine when to use request_irq */
static int ATMEL_SyncWithThreadToReadMsg(struct mxt_data *mxt, int boot)
{
	int error, count = 0;;
	...
	if (boot) {
                ...
                while(!irq_coming)
		{
			...
			if(count == 20)
			{
				mxt_debug(DEBUG_ERROR,"maXTouch1386: no interrupt, there may be no device or crashed\n");
				up(&mxt->sema);
				return -1;
			}
		}
...
void mxt_early_suspend(struct early_suspend *h)
{
	...
	*/
	if (ATMEL_Deepsleep(data) == 0) {
		mxt_debug(DEBUG_ERROR, "maXTouch1386: ATMEL_Deepsleep OK! ret:%d\n", ret);
	} else {
